...
function generateRelationshipGraph(movieSelected) {
  ...
  elements.push({
    data: { id: movieSelected.id, label: movieSelected.title, type: "main-movie", image: movieSelected.image }
  });

  if (movieSelected.directorId) {
    elements.push({ data: { id: movieSelected.directorId, label: movieSelected.directorName, type: "director" } });
  ...
  const cy = cytoscape({
    ...
    elements: elements,
    style: [
      { selector: "node", style: { label: "data(label)", "text-valign": "bottom", "text-margin-y": 8, color: "#fff", "font-size": "10px", "background-color": "#444", width: "40px", height: "40px", "background-fit": "cover", "border-width": 2, "border-color": "#555" } },
